...
START_TEST(test_2d_area_triangle)
{
    ...
    c.x = 50;
    c.y = 25;
    got = coord_2d_area_triangle(&a, &b, &c);
    exp = 212.50;
    ck_assert(coord_2d_double_eq(got, exp));
...
Suite* coord_2d_suite(void)
{
    ...
    TCase* tc_2d_area_trianle = tcase_create("coord_2d_area_triangle");
    tcase_add_test(tc_2d_area_trianle, test_2d_area_triangle);

    /* Add Cases to Suite */
    suite_add_tcase(s, tc_2d_eq);
    suite_add_tcase(s, tc_2d_dist);
    suite_add_tcase(s, tc_2d_midpoint);
    suite_add_tcase(s, tc_2d_area_trianle);
